/* VENDORED from dashboard/app/static/liquid-glass-webgl.js - the dashboard's
 * liquid-glass WebGL material, reused for the tray popover so both surfaces
 * share the exact same shader. Only OBJECT_SELECTOR differs (see below).
 * The backdrop texture comes from --bg/--bg-end plus an optional
 * .workspace-photo-panel element, both provided by glass-backdrop.js. */
...
(() => {
  ...
  // Tray adaptation: the only glass target is the popover panel. Everything
  // else in this file is byte-identical to the dashboard module.
